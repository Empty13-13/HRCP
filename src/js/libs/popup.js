    ...
    let config = {
      ...
      on: { // События
        ...
        beforeOpen: function (popup) {
          ...
          if (popup.hash === "#editAddress") {
            let originalTable = popup.lastFocusEl.parentNode.querySelector('.adress-left-info-submit-trash__table')
            let popupTable = document.querySelector('#popupTable')
            if (originalTable && popupTable) {
              popupTable.innerHTML = originalTable.innerHTML
              this.lastTable = originalTable.innerHTML
              originalTable.innerHTML = ""
            }
          }
          // region closeEditAdressBtn
          let closeEditAdressBtn = document.querySelector('#closeEditAdressBtn')
          if(closeEditAdressBtn) {
            let lol = this
            closeEditAdressBtn.addEventListener("click",function(e) {
              lol.isBtnClose = true
            });
          }
          // endregion
          // region edit popup label
          let tablePopup = document.querySelector('#popupTable')
          if (tablePopup) {
            let labels = tablePopup.querySelectorAll('label')
            if (labels.length) {
              labels.forEach(item => {
                if (!item.classList.contains('options__label')) {
                  let textarea = document.createElement('textarea')
                  textarea.classList.add('input')
                  textarea.autocomplete = false
                  textarea.innerHTML = item.innerText
                  item.parentNode.append(textarea)
        ...
        afterClose: function (popup) {
          if (popup.hash === "#editAddress") {
            if(!this.isBtnClose){
              ...
              let popupTable = document.querySelector('#popupTable')
              if (originalTable && popupTable) {
                originalTable.innerHTML = this.lastTable
              ...
              if (tablePopup) {
                ...
                if (labels.length) {
                  labels.forEach(item => {
                    if (!item.classList.contains('options__label')) {
                      let textarea = item.parentNode.querySelector('textarea')
                      item.hidden = false
              ...
              let popupTable = document.querySelector('#popupTable')
              if (originalTable && popupTable) {
                originalTable.innerHTML = popupTable.innerHTML
                popupTable.innerHTML = ""
              }
            }
          }
