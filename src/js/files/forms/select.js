...
import {filterByItem} from "../script.js"
...
export const selectModule = {
  ...
  optionAction(selectItem, originalSelect, optionItem) {
    if (originalSelect.multiple) { // Если мультивыбор
      ...
      if (originalSelect.hasAttribute('data-sorting')) {
        if (optionItem.innerHTML) {
          filterByItem(optionItem.innerHTML)
        }
      }
  ...
  searchActions(selectItem) {
    ...
    const selectOptions = selectModule.getSelectElement(selectItem, selectModule.selectClasses.classSelectOptions).selectElement;
    let selectOptionsItems = selectOptions.querySelectorAll(`.${selectModule.selectClasses.classSelectOption}`);
    ...
    selectInput.addEventListener('keydown', (e) => {
      if (e.key === "Enter") {
        ...
        option.innerText = selectInput.value
        option.value = originalSelect.options.length + 1

        originalSelect.append(option)
        selectModule.setOptions(selectItem, originalSelect)
        selectOptionsItems = selectOptions.querySelectorAll(`.${selectModule.selectClasses.classSelectOption}`);
        selectModule.optionAction(selectItem, originalSelect, selectOptionsItems[selectOptionsItems.length - 1])
      }
    })
  },
  // Коллбэк функция
